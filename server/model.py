...
# def predict_by_year(input_year,model):
#     state_names = {
...
# Check if processed file exists, else run the function
input_file = 'us_disaster_declarations.csv'
output_file = 'processed_data.csv'
if not os.path.exists(output_file):
    process_data_and_save_to_csv(input_file, output_file)
...
# Function to get the appropriate model based on the input parameter
...
def predict_by_year(input_year, model_name):
    ...
    for state in states:
        ...
        for incident in common_incidents:
            ...
            prediction_value = float(round(probabilities[0][1] * 100, 2))  # Convert float32 to float
            probability_occurrence = prediction_value
            avg_pred_total += prediction_value
            state_predictions["predictions"][incident] = probability_occurrence
        state_predictions["predictions"]["Avg"] = float(round(avg_pred_total / len(common_incidents), 2))  # Convert float32 to float
        predictions.append(state_predictions)
